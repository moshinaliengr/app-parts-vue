...
const newVerification = (requestOptions) => {
  const { auth, payload } = requestOptions;
  return userClient(auth).post(`api/user/verify/email`, payload);
};

const register = async (requestOptions) => {
  const { payload } = requestOptions;
...
const deleteUserAccount = (requestOptions) => {
  const { auth } = requestOptions;
  return userClient(auth).delete(`/api/v1/user`);
};
